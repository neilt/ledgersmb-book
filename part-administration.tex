...
Go to the \menupath{System \ma Users}. You'll be presented the page as shown in figure \figref{fig:create-user-step1}.
...
by navigating to \menupath{System \ma Users} which brings you to the same screen as presented in
...
See also \secref{subsec-create-setup-login}
...
\item [Currency] The currency\index{currency} belonging to the Last cost field
\end{description}
...
\subsection{Parts versus assemblies for multi-item-package sales}
...
Next to the regular \lsmbsp G/L account numbering scheme, LedgerSMB supports a second
numbering scheme:  \gls{gifi} numbering. \gls{gifi}\index{gifi} is a standardized account\index{account number alternatives}
coding system used in Canada by the Canada Revenue Agency\index{Canada Revenue Agency} for processing
corporate Tax Returns forms. 

Other countries (e.g. France\index{France}) may have required numbering schemes for
corporate accounting as well and the GIFI account functionality can be used for them also.

If you use GIFI account numbers, each \lsmbsp G/L account is associated with a GIFI account. 
Multiple G/L accounts may map to a single GIFI account.
Note that the hierarchy for G/L accounts is completely separate from GIFI accounts.

General Ledger reports may exist in two variants: a variant using the
normal G/L accounts and one with the GIFI numbering. 
In the GIFI variant, when a single GIFI account is mapped to multiple \lsmbsp accounts, the total reported
under GIFI is the sum of the mapped G/L accounts.

The GIFI reporting variant only shows if GIFI accounts are defined. 

\subsection{Maintaining GIFI}
\label{subsec-coa-gifi-maintenance}

GIFI accounts must be created before they can be assigned to the \lsmbsp G/L account.
Some predefined Chart of Accounts define GIFI accounts and map the association of GIFI to G/L accounts.

GIFI accounts can be reviewed, lightly edited, entered or imported by navigating to \menupath{General Journal \ma GIFI}.

Once GIFI accounts are defined the account number cannot be changed.  However, the GIFI description can be changed any time.
Be aware that changing this description will effect reports generated on historical data prior to this change.

\lsmbsp G/L accounts can be assigned to GIFI accounts by navigating to \menupath{General Journal \ma Chart of Accounts} and
selecting a G/L account. In the resulting view, the Configuration section shows the GIFI dropdown.

G/L account headings cannot be assigned to GIFI accounts.


\section{Reporting}
\label{sec-coa-reporting}

As for the general journal being presented in the "ordered by account" and "ordered by date": these are both in the \menupath{General Journal \ma Search} menu. 
...
I believe that in the \menupath{General Journal \ma Search} search options, you may need to check the "Account" indicator to add the account to the output. This comes in handy when you decide not to filter for a specific account.
...
Tax forms can be created by navigating to \menupath{Tax Forms \ma Add Tax Form}.

Tax forms can be edited by navigating to \menupath{Tax Forms \ma List Tax Forms}.
...
file, you should use the migration utility\\
\texttt{utils/migration/convert-ini-to-di}
...
login, the question \\
"LedgerSMB X.YY db found. Rebuild/Upgrade?" with a \texttt{Yes}
