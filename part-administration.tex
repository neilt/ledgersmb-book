...
numbering scheme: GIFI numbering. \gls{gifi}\index{gifi} is a standardized account\index{account number alternatives}
...
If you use GIFI account numbers, each \lsmbsp account is associated with a GIFI
account. Multiple \lsmbsp accounts may map to a single GIFI account.
...
GIFI accounts must be created before being assigned to the \lsmbsp G/L account. 
GIFI accounts can be maintained by navigating to
 \menupath{General Journal \ma Chart of accounts \ma GIFI}.

\lsmbsp G/L accounts can be assigned to GIFI accounts by selecting the \lsmbsp G/L account in the "Chart of Accounts" view
and updating the drop down.
Keep in mind that \lsmbsp G/L account headings cannot be assigned to GIFI accounts.
