...
right to create databases and other users; the \texttt{postgres}\index{postgres}\index{user postgres} super user has these rights
and more, so it's generally convenient (but not very secure) to use it.

You might also see the user name \texttt{lsmb\_dbadmin}\index{lsmb\_dbadmin}\index{user lsmb\_dbadmin}.
This user is hard-coded as an alternate to 
\texttt{postgres} because setup.pl can't connect to the database before it receives a valid
user name and password. To use this user name it has to be setup during the installation
process. This is not done by default, but is mentioned in some installation instructions as a more 
secure alternative and should be used if available. Also see \secref{sec-users-management-initial}.
...
If this screen is not shown then Jack navigates to the login screen at\\
\url{https://example.com/ledgersmb/login.pl}.
...
Next Jack navigates to \menupath{System \ma Country} and sets the default country to \texttt{United States} by clicking on the radio button.

And finally Jack needs to set the default language by navigating to \menupath{System \ma Language} and clicking the radio button for \texttt{English (US)}.
...
Knowing this, Jack looks up the example bank account from his previously configured US chart of
...
\clearpage
\section{Creating initial bank balance}
\label{sec-initial-bank-balance}

When a company is formed there is an initial investment. This varies by the type of company.

Jack's company was formed as a corporation. 
The initial purchase of the company's  common shares needs to be entered
into the bank account\index{bank account} to establish the initial balance\index{bank account, initial balance}.

To do this Jack will create a manual Journal Entry which will debit the Checking Account (1060) and credit the
Common Shares account (3350). Jack's initial investment was \$5000.00.

To do this Jack will:
\begin{enumerate}
\item Navigate to \menupath{General Journal \ma Journal Entry}
\item Enter ``0001`` into the Reference field
\item Enter ``Initial Investment`` into the Description field
\item Select account 1060 in the first row of the Account column
\item Select account 3360 in the second row of the Account column
\item Enter ``5000`` under in Debit column for account 1060
\item Enter ``5000`` under the Credit column for account 3360
\item In the Source column for each row enter the source. This could be a check number, but our purposes we are going to enter ``EFT`` for \gls{EFT}
\item The screen should look like the screen show in \figref{fig:bank-setup3}
\item Click ``Update``
\item The screen should look like the screen shown in  \figref{fig:bank-setup4}
\item Click ``Save`` to save the transaction
\item The screen should look like the screen shown in \figref{fig:bank-setup5}
\item Click the ``Post`` button to approve the transaction
\item To confirm the initial balance navigate to \menupath{General Journal \ma Reports \ma Balance Sheet}
\item Observe the new \$5000.00 balance in the Current Assets section and the \$5000.00 balance in the 
Share Capital section of the balance sheet report as shown in \figref{fig:bank-setup6}
\end{enumerate}

\begin{figure}[hbt!]
	\centering
	% \includegraphics[width=\linewidth]{setup-bank-account2.png}
	\includegraphics[width=\linewidth]{images/initial-bank-balance.png}
	\caption{Bank account setup - initial bank balance}
	\label{fig:bank-setup3}
\end{figure}

\begin{figure}[hbt!]
	\centering
	% \includegraphics[width=\linewidth]{setup-bank-account2.png}
	\includegraphics[width=\linewidth]{images/initial-bank-balance2.png}
	\caption{Bank account setup - initial bank balance after update}
	\label{fig:bank-setup4}
\end{figure}

\begin{figure}[hbt!]
	\centering
	% \includegraphics[width=\linewidth]{setup-bank-account2.png}
	\includegraphics[width=\linewidth]{images/initial-bank-balance3.png}
	\caption{Bank account setup - initial bank balance after save}
	\label{fig:bank-setup5}
\end{figure}

\begin{figure}[hbt!]
	\centering
	% \includegraphics[width=\linewidth]{setup-bank-account2.png}
	\includegraphics[width=\linewidth]{images/initial-bank-balance4.png}
	\caption{Bank account setup - balance sheet confirmation}
	\label{fig:bank-setup6}
\end{figure}
...
\subsection{Single-item \& ``multi-item pre-packaged''}
...
\begin{quote}
\menupath{Goods and Services \ma Part Groups}.
\end{quote}
For each group in the list below Jack enters the group name and clicks the green check mark after each one:
...
Using the work flow detailed in \secref{sec-business-processes-creating-customers-and-vendors} Jack starts to do so by going through the menu \menupath{Contacts \ma Add Entity \ma Company}.
...
\gls{rfq} he sent to the vendor using the menu path \menupath{Quotations \ma Reports \ma Quotation}.
...
Jack is going to setup a company as a customer and starts by navigating to add contact screen using the menu path \menupath{Contacts \ma Add Entity \ma Company}. 
...
\begin{longtable}{ llp{6cm} }
    ...
    'Pay To & \texttt{ABC, Inc.} & If printed checks are used, then this line is the Pay To line on the check. It also shows up when picking a customer with multiple credit accounts\\
    & & \\
...
\subsection{Pardoning and underpayment}
\label{subsec-sales-payment-mismatch}

\subsection{Handling large mismatches}
\label{subsec-sales-payment-partial}

 As in partial payments or largish under/over payments
...
\subsection{Voiding checks}

Voiding checks to undo payments of vendor invoices relating to bounced checks
