...
\item [Pay To] If printed checks are used, then this line is the Pay To line on the check. It also shows up when picking a customer with multiple credit accounts
...
\item [Currency] The default currency\index{currency} to be used with the \gls{customer} or vendor
...
The \gls{over payment}\index{over payment} functionality in \lsmbsp is used for handling customer or vendor deposits\index{deposit}, 
prepayments\index{prepayment}, advances\index{advances}, etc.  
Essentially any activity that results in paying too much or in advance.
...
invoices in foreign\index{currency} currencies\footnote{This pertains exclusively
