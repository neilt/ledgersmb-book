...
For an example of the default, non debug \texttt{ledgersmb.yaml} \index{ledgersmb.yaml} see the file in the \href{https://github.com/ledgersmb/LedgerSMB/blob/master/doc/conf/ledgersmb.yaml}{Github Repository}.
...
\begin{description}
    ...
    \begin{description}
        ...
        \item {\texttt{sasl\_username:mechanism}} –  The available mechanism are defined at\\
         	\url{https://metacpan.org/dist/Authen-SASL} or \\
         	\url{ https://metacpan.org/dist/Authen-SASL-SCRAM} depending on the selected `\$class`.
...
LedgerSMB provides an \gls{XSD} \index{XSD}\index{CoA XSD}file, which might help in developing a custom CoA XML file,  located in the\\ \href{https://github.com/ledgersmb/LedgerSMB/blob/master/doc/company-setup/configuration.xsd}{Github Repository doc/company-setup/configuration.xsd}.  
...
\href{https://github.com/ledgersmb/LedgerSMB/blob/master/locale/coa/us/GeneralHierarchical.xml}{GitHub Repository locale/coa/us/GeneralHierarchical.xml}.
...
For example, \menupath{General Journal \ma Reports \ma Income Statement} as shown in \figref{fig:config-report-hierarchy}.
...
\subsubsection{Base currency}
\label{subsubsec-company-config-defaults-currencies}

Select the base currency\index{currency}
to use in your company, identified by their 3-letter codes in the drop down. 

If the desired currency is not listed then navigate to \menupath{ System \ma Currency \ma Edit Currencies}.

All currencies listed in the Defined Currencies view will be shown for use in \lsmb.

Add the new currency and delete any unwanted currencies. Then return to \menupath{System \ma Defaults} and select the newly added currency.
...
the \menupath{Goods \& Services \ma Warehouses} menu item.
...
SIC \index{SIC}codes feature these three fields:
...
When creating a company you can assign that it an \gls{SIC} code, irrespective of its role (i.e. customer,
...
In \texttt{setup.pl} there is also a button to upload all Demo templates into the database. These templates will replace all templates in the database.

There are 3 types of templates: \LaTeX (tex), HTML (html), and CSV (csv).
...
\begin{description}
	\item [Edit] allows editing and saving the database version of the template.
	\item [Upload] copies a local version of a template into the database.
\end{description}
...
% Merge https://github.com/ledgersmb/LedgerSMB/blob/master/doc/templating/templating-guide.tex
% On 2025-01-05

\subsubsection{Template Syntax}
\label{subsec-company-config-templates-syntax}
\index{template syntax}

\newcommand{\pitag}[1]{\textless?lsmb #1 ?\textgreater}
\newcommand{\lsmbt}{LedgerSMB::Template}

Templating directives in are expressed with the base
form of \pitag{foo}.  That, and basic variable substitution, is about the only
part of templating that has remained unchanged.  Starting with \lsmb 1.3, the primary
rendering engine is Template Toolkit, with START\_TAG changed to
``\textless?lsmb'' and END\_TAG changed to ``?\textgreater''.

\subsubsection{Variable Substitution}
\label{subsec-company-config-templates-variable-sub}
\index{template var substitution}
...
that in some cases, the key name conflicts with one of Template Toolkit's VMethods for the
...
\label{subsec-company-config-templates-internationalization}

In addition to the regular Template Toolkit functions, LedgerSMB has the
...
\label{subsec-company-config-templates-formatting}
...
\label{subsec-company-config-templates-control}
...
\label{subsec-company-config-templates-debugging}
\index{template debugging}

Two template-side tools that can help with debugging are the interface to
Data::Dumper and setting a debug message format.

\textbf{Variable Dumping}
\index{template var dumping}
...
\subsubsection{Templating API}
\label{subsec-company-config-templates-api}
\index{template api}

A new templating API was added in LedgerSMB 1.3.  This API is accessed through
...
\subsubsection{Support Modules}
\label{subsec-company-config-templates-modules}
...
A format infrequently used by \lsmb, this is used for Point Of Sale invoices.
It looks for templates that end in ``.txt''.

\textbf{\lsmbt::TTI18N}

This module defines some internationalization functions that can be called in
...
\label{subsec-company-config-templates-quick}
\index{template quick reference}
...
\subsubsection{Templating Changes Between Versions}
\label{subsec-company-config-templates-changes}

\textbf{\lsmb 1.3 to \lsmb 1.2}
...
\begin{itemize}
	...
	\item The \lsmb::Mailer API was changed and documented
\end{itemize}

\newcommand{\tg}{\textgreater}
\newcommand{\tl}{\textless}

\textbf{\lsmb 1.2 to \lsmb 1.1}
\begin{itemize}
	...
	\item \lsmb::Mailer was rewritten to use MIME::Lite while retaining the old API
	\item SMTP support was added to \lsmb::Mailer
\end{itemize}
