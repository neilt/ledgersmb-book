...
LedgerSMB is an open source, web based \gls{ERP} system. Which is a system
which supports business processes of all disciplines throughout the
organization, automating as much of those processes as possible. 
...
listed at\\
\url{https://ledgersmb.org/topics/support}.
...
As most \gls{ERP} systems, LedgerSMB's functionalities are grouped into modules.
...
to create a \gls{REST}\index{REST} web service\index{web service} based \gls{API}\index{API}.
More \gls{API} information is available in \charef{cha-customization-web-service-api}.
The intent here is to facilitate
...
The \gls{ias} and\\ 
\gls{ifrs} describe requirements
for financial statements (reports) and the underlying accounting process
