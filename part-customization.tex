...
e-mail messages.  For a list of available transports, see\\
...
this can be achieved by implementing a custom\\
\texttt{Email::Sender::Transport}.
...
\chapter{Web service APIs}
\label{cha-customization-web-service-api}

LedgerSMB comes with a web service\index{web service API} \gls{API}\index{API}. 
The version number assigned follows the rules of \href{https://semver.org/}{semantic versioning}. 

The current major version is 0 (zero), meaning that it's not considered to have stabilized
...
The API is hosted on \texttt{erp/api/v0}, on the same root as \texttt{login.pl}. 

That is to say that if \lsmb's login screen is hosted at

\texttt{https://example.org/login.pl} \\

then the API can be found at

\texttt{https://example.org/erp/api/v0}.  \\

The definitive API definition is in the source code located at
\texttt{doc/openapi/API.yaml}.

There is also a web reference version at \\
\url{https://docs.ledgersmb.org/openapi/1.13.0/LedgerSMB-api.html}

Note that the above URL is \lsmbsp version dependent. There may be newer versions available and
you should refer to the version that matches your installation.
