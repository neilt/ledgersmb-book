...
% ---------------------------
% To do's are represented by the string @@@TODO or @@@ TODO
%
...
%        \end{longtable}
%
% Remarks or notes should be formatted as follows:
%    \begin{quote}
...
% Package hyperref colors are ignored in HTML
% Black color means no visible indication of the link in the online PDF
...
\hypersetup{
        colorlinks, % Colors instead of ugly boxes
        citecolor=red, % citations, default green. Not used and we are not building a christmas tree
        filecolor=red, % urls to open local files, default cyan. Not used and we are not building a christmas tree
        linkcolor=violet, % internal links, default red, violet seems less shocking
		urlcolor=magenta  % external hyperlinks, use default magenta
}
...
% the menu path format
\newcommand{\menupath}[1]{{\tt #1}}
% the menu path separator
...
Because LedgerSMB is undergoing active infrastructure and API changes in 2026, some portions of this book will not be written until those changes are nearing completion.
...
administrators.
It offers something for everybody who works with or considers working with \lsmb.
...
The content of this book is being developed in the ledgersmb-book Github project at
\url{https://github.com/ledgersmb/ledgersmb-book}. \\
~ \\
Comments and enhancement ideas can be filed in the Github issue tracker. 
Additionally, discussion about the content
can be organized on the mail list or chat channels listed at \url{https://ledgersmb.org/topics/support}.\\
~ \\
We desperately need to finish adding content. Feel free to suggest new or changes to the content
using the Github issue tracker. Don't worry about knowing details of \LaTeX{} or how the
content should be structured. We'll take care of that.\\
~ \\
Up-to-date versions of this book in both PDF and HTML formats can be found on the book website at \url{https://book.ledgersmb.org}.\\
~ \\
% Different style here because the base link is too long for the line
General information about the community around \lsmbsp can be found in the source repository contributing document
\href{https://github.com/ledgersmb/LedgerSMB/blob/master/CONTRIBUTING.md}{CONTRIBUTING.md}

\section*{Other information sources}
\label{sec-other-sources}
\addcontentsline{toc}{section}{Other information sources}

The following sources of information, while not maintained by the LedgerSMB project, might be helpful in understanding the project.

\url{https://deepwiki.com/ledgersmb/LedgerSMB} Contributed by jgart.
 
\url{https://grokipedia.com/page/ledgersmb}

\url{https://en.wikipedia.org/wiki/LedgerSMB}
